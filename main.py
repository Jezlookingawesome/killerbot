...
async def checkchamber(ctx):
    ...
    if current == 1:
        await smart_reply(ctx, format_quote(
            ctx.guild.id,
            "🔍 Chamber check: 1 bullet out of 6.",
            "🔍 CHAMBER CHECK: 1 BULLET OUT OF 6.",
        ))
    else:
        await smart_reply(ctx, format_quote(
            ctx.guild.id,
            f"🔍 Chamber check: {current} bullets out of 6.",
            f"🔍 CHAMBER CHECK: {current} BULLETS OUT OF 6.",
        ))
...
@bot.command()
async def gamblingmode(ctx):
    current = gambling_mode.get(ctx.guild.id, False)
    new_state = not current
    gambling_mode[ctx.guild.id] = new_state

    if new_state:
        await ctx.send("Gambling mode activated. 🎰🎰👀")
    else:
        await ctx.send("Gambling mode deactivated.")


@bot.command()
async def ping(ctx):
    latency_ms = round(bot.latency * 1000)
    await smart_reply(ctx, f"LATENCY: {latency_ms}MS")
